...
// NewRouterWithNetworkSupport creates a router with network switching support
func NewRouterWithNetworkSupport(networkHandler *NetworkHandler, defaultSDK SDKInterface, cfg *config.Config) *gin.Engine {
	handler := &Handler{
		SDK:            defaultSDK,
		NetworkHandler: networkHandler,
		Config:         cfg,
...
<body>
	...
	// Backend-only endpoint with network support
	rewards.POST("/claim-custom", handler.ClaimCustomRewardV2WithNetwork)
