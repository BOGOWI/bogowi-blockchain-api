...
contract BOGOTokenV2_ZeroValidated is ERC20, AccessControl, Pausable, ReentrancyGuard {
    ...
    mapping(bytes32 => address) private flavoredTokensByHash;
    ...
    /**
     * @dev Queue flavored token registration with comprehensive validation
     * @param flavor The flavor identifier
     * @param tokenAddress The token contract address (must not be zero and must be a contract)
     */
    ...
    {
        if (!_isContract(tokenAddress)) revert NotAContract();
    ...
    /**
     * @dev Execute flavored token registration after timelock
     * @param flavor The flavor identifier
        ...
        onlyRole(DEFAULT_ADMIN_ROLE) 
    {
        ...
        // Store hash mapping for gas optimization
