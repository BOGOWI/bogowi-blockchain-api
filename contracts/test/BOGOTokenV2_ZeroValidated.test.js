...
describe("BOGOTokenV2_ZeroValidated", function () {
  ...
  beforeEach(async function () {
    ...
    // Deploy a mock ERC20 token for testing flavored tokens
    ...
    beforeEach(async function () {
      operationId = ethers.keccak256(
        ethers.solidityPacked(
          ...
          ["registerFlavoredToken", flavor, await mockToken.getAddress()]
        )
      );
    });

    describe("Queue Flavored Token Registration", function () {
      it("Should reject zero address token", async function () {
        await expect(
          bogoToken.queueRegisterFlavoredToken(flavor, ethers.ZeroAddress)
        ).to.be.revertedWithCustomError(bogoToken, "InvalidAddress");
      });

      it("Should reject non-contract address", async function () {
        await expect(
          bogoToken.queueRegisterFlavoredToken(flavor, await user1.getAddress())
        ).to.be.revertedWithCustomError(bogoToken, "NotAContract");
      });

      it("Should queue valid registration", async function () {
        const currentTime = await time.latest();
        
        await expect(
          bogoToken.queueRegisterFlavoredToken(flavor, await mockToken.getAddress())
        ).to.emit(bogoToken, "TimelockQueued");

        const executeTime = await bogoToken.timelockOperations(operationId);
        expect(executeTime).to.be.greaterThan(currentTime);
        expect(executeTime).to.equal(currentTime + TIMELOCK_DURATION + 1); // +1 for block time
      });

      it("Should only allow admin to queue", async function () {
        await expect(
          bogoToken.connect(user1).queueRegisterFlavoredToken(flavor, await mockToken.getAddress())
        ).to.be.revertedWithCustomError(bogoToken, "AccessControlUnauthorizedAccount");
      });
    });

    describe("Execute Flavored Token Registration", function () {
      beforeEach(async function () {
        await bogoToken.queueRegisterFlavoredToken(flavor, await mockToken.getAddress());
      });

      it("Should reject execution before timelock expires", async function () {
        await expect(
          bogoToken.executeRegisterFlavoredToken(flavor, await mockToken.getAddress())
        ).to.be.revertedWithCustomError(bogoToken, "TimelockNotExpired");
      });

      it("Should reject execution of non-queued operation", async function () {
        await expect(
          bogoToken.executeRegisterFlavoredToken("chocolate", await mockToken.getAddress())
      ...
      it("Should execute after timelock expires", async function () {
        ...
        await expect(
          ...
          .withArgs(flavor, await mockToken.getAddress())
          .and.to.emit(bogoToken, "TimelockExecuted");

        expect(await bogoToken.flavoredTokens(flavor)).to.equal(await mockToken.getAddress());
        
        // Check hash mapping
        const flavorHash = ethers.keccak256(ethers.toUtf8Bytes(flavor));
        expect(await bogoToken.getFlavoredTokenByHash(flavorHash)).to.equal(await mockToken.getAddress());
        
        // Operation should be cleared
        expect(await bogoToken.timelockOperations(operationId)).to.equal(0);
      });

      it("Should only allow admin to execute", async function () {
        await time.increase(TIMELOCK_DURATION + 1);
        
        await expect(
          bogoToken.connect(user1).executeRegisterFlavoredToken(flavor, await mockToken.getAddress())
        ).to.be.revertedWithCustomError(bogoToken, "AccessControlUnauthorizedAccount");
      });
    });

    describe("Cancel Timelock Operation", function () {
      beforeEach(async function () {
        await bogoToken.queueRegisterFlavoredToken(flavor, await mockToken.getAddress());
      });
      ...
      it("Should reject cancelling non-queued operation", async function () {
        const fakeOperationId = ethers.keccak256(ethers.toUtf8Bytes("fake"));
        
        await expect(
          bogoToken.cancelTimelockOperation(fakeOperationId)
        ).to.be.revertedWithCustomError(bogoToken, "OperationNotQueued");
      });
  ...
  describe("Access Control", function () {
    ...
    it("Should enforce admin role for timelock operations", async function () {
      await expect(
        bogoToken.connect(user1).queueRegisterFlavoredToken("vanilla", await mockToken.getAddress())
      ).to.be.revertedWithCustomError(bogoToken, "AccessControlUnauthorizedAccount");
    });
  ...
  describe("Edge Cases", function () {
    ...
    it("Should handle multiple flavored token registrations", async function () {
      const flavors = ["vanilla", "chocolate", "strawberry"];
      
      for (const flavor of flavors) {
        await bogoToken.queueRegisterFlavoredToken(flavor, await mockToken.getAddress());
        await time.increase(TIMELOCK_DURATION + 1);
        await bogoToken.executeRegisterFlavoredToken(flavor, await mockToken.getAddress());
  ...
  describe("Gas Optimization Features", function () {
    it("Should store flavored tokens by hash for gas optimization", async function () {
      const flavor = "vanilla";
